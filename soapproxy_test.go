...
func TestTrimOuterTag(t *testing.T) {
	for tN, tC := range map[string]struct {
		In, Out, Tag string
	}{
		"simple": {
			In: `
		<PERSON_ITEMS><GDPRPerson><StartDate>1953-04-20</StartDate><LastName>Andras</LastName><FirstName>Huszti</FirstName><BirthCity>BUDAPEST</BirthCity><MotherLastName>Kiss Gizi</MotherLastName></GDPRPerson>

<GDPRPerson><ClaimList><GDPRClaim><ID>KKCL-009027228T-1</ID></GDPRClaim></ClaimList></GDPRPerson>
<GDPRPerson><EMailAddressList><GDPREMailAddress><Text>hajdumarcsi.580303@googmail.hu</Text></GDPREMailAddress></EMailAddressList><SimplifiedContractList><GDPRSimplifiedContract><ContractId>365128</ContractId></GDPRSimplifiedContract></SimplifiedContractList></GDPRPerson>
<GDPRPerson><EMailAddressList><GDPREMailAddress><Text>muranyi.jozsef@hdsnet.hu</Text></GDPREMailAddress></EMailAddressList><SimplifiedContractList><GDPRSimplifiedContract><ContractId>31685769</ContractId></GDPRSimplifiedContract></SimplifiedContractList></GDPRPerson></PERSON_ITEMS>
`,
			Tag: "PERSON_ITEMS",
			Out: `<GDPRPerson><StartDate>1953-04-20</StartDate><LastName>Andras</LastName><FirstName>Huszti</FirstName><BirthCity>BUDAPEST</BirthCity><MotherLastName>Kiss Gizi</MotherLastName></GDPRPerson>
...
<GDPRPerson><EMailAddressList><GDPREMailAddress><Text>muranyi.jozsef@hdsnet.hu</Text></GDPREMailAddress></EMailAddressList><SimplifiedContractList><GDPRSimplifiedContract><ContractId>31685769</ContractId></GDPRSimplifiedContract></SimplifiedContractList></GDPRPerson>`,
		...
		"xmlns": {
			In: `<gdpr:GDPRRequest xmlns:gdpr="http://aegon.hu/exampl/GDPR">
			<GDPR_REQUEST_HEADER /></gdpr:GDPRRequest>`,
			Tag: "gdpr:GDPRRequest",
			Out: "<GDPR_REQUEST_HEADER />",
		},
		...
		t.Run(tN, func(t *testing.T) {
			t.Parallel()
			tag, out := trimOuterTag([]byte(tC.In))
			if tag != tC.Tag {
				t.Errorf("got tag %q, wanted %q", tag, tC.Tag)
			}
			if string(out) != tC.Out {
				t.Errorf("got %q,\nwanted %q", string(out), tC.Out)
			}
		})
	}
}
